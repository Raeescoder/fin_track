/* ================================================================
   ...
   All page renderers read from AppState — never from the DOM.

   To connect a real backend:
   1. Replace the arrays below with API fetch calls
   2. Call the relevant renderXxxPage() after data loads
   3. Keep the AppState shape the same so render functions
      don't need to change.

   Contents:
   1. AppState — global mutable state object
   2. CAT_ICONS — emoji map for transaction categories
   3. AI_RESPONSES — rotating canned AI advisor answers
   ================================================================ */


/* ── 1. AppState ─────────────────────────────────────────────────
   Single global object that holds all app state.
   Mutated directly by modal handlers (addTransaction, etc.)
   and read by all render functions.
   ──────────────────────────────────────────────────────────────── */
var AppState = {

  /* Whether the user has upgraded to Pro.
     Set to true by completePurchase() in modals.js.
     Controls premium gate visibility on Balance Sheet & Reports. */
  isPro: false,

  /* Tracks which page is currently visible.
     Updated by router.js on every navigation. */
  currentPage: 'dashboard',

  /* ── Transactions ──────────────────────────────────────────────
     Each transaction object shape:
     {
       id:     number   — unique identifier (Date.now() for new ones)
       desc:   string   — human-readable description
       amount: number   — always positive; sign determined by type
       type:   string   — "Income" | "Expense"
       cat:    string   — category name (must match CAT_ICONS keys)
  ...
  /* ── Budgets ───────────────────────────────────────────────────
     Each budget object shape:
     {
       cat:   string  — category name (matches transaction cats)
       limit: number  — monthly spending limit in USD
       spent: number  — amount spent so far this month
  ...
  /* ── Accounts ──────────────────────────────────────────────────
     Each account object shape:
     {
       name:    string  — display name
       type:    string  — "Checking" | "Savings" | "Credit Card" | "Investment" | "Loan"
       balance: number  — current balance (negative = liability/debt)
       icon:    string  — emoji representing account type
     }
     ──────────────────────────────────────────────────────────── */
  accounts: [
    { name:'Chase Checking',     type:'Checking',    balance:8240,   icon:'🏦' },
    { name:'High-Yield Savings', type:'Savings',     balance:22800,  icon:'💰' },
    { name:'Chase Sapphire',     type:'Credit Card', balance:-3280,  icon:'💳' },  /* Negative = owe money */
    { name:'Vanguard 401k',      type:'Investment',  balance:18200,  icon:'📈' },
    { name:'Car Loan',           type:'Loan',        balance:-12500, icon:'🚗' },  /* Negative = debt */
  ],

  /* ── Admin: Users ──────────────────────────────────────────────
     Mock user data for the admin panel user management table.
     In a real app this would come from a /admin/users API endpoint.
     ──────────────────────────────────────────────────────────── */
  adminUsers: [
  ...
  /* ── Admin: Recent Payments ────────────────────────────────────
     Mock payment history for the admin payments table.
     ──────────────────────────────────────────────────────────── */
  adminPayments: [
...
/* ── 2. Category Icon Map ─────────────────────────────────────────
   Maps transaction category names to emoji icons.
   Used by dashboard tx preview and transactions table.
   Add new categories here when adding new <option> values.
   ──────────────────────────────────────────────────────────────── */
var CAT_ICONS = {
  'Food & Dining': '🍔',
  'Housing':       '🏠',
...
/* ── 3. AI Canned Responses ──────────────────────────────────────
   Rotating responses shown in the AI chat panel.
   AI_IDX tracks which response to show next (wraps around).
   In a real app, replace showAIChat() with an API call to
   an LLM endpoint and remove these static responses.
   ──────────────────────────────────────────────────────────────── */
var AI_RESPONSES = [
...
/* Index pointer — incremented each time showAIChat() is called */
var AI_IDX = 0;
